...
def seed_channels():
    demo_channels = [
      ...
      Channel(
        server_id=2, name='places-to-visit', topic='Recommend places to visit in Japan'
        ),
      Channel(
        server_id=2, name='must-try-foods', topic='Restaurants we must try in Japan'
        ),
      Channel(
        server_id=3, name='general', topic='<h1> Hello Programmers </h1>'
        ),
      Channel(
        server_id=3, name='coding-help', topic='Stuck? Ask for help!'
        ),
      Channel(
        server_id=3, name='css-resources', topic='Share resources that helped you!'
        ),
      Channel(
        server_id=4, name='general', topic='Share all things Ghibli!'
        ),
      Channel(
        server_id=4, name='soundtrack', topic='Share soundtrack favorites from the films'
        ),
      Channel(
